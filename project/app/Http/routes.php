...
Route::group(['middleware' => 'web'], function () {
    ...
    })->name('administracion')->middleware('guest');
    // el middleware guest hace redireccion a /listado-egresados o /admin-publicaciones si está logueado
    ...
    Route::get('/publicaciones-list/{categoria?}','PublicacionesController@lista')
        ->name('publicaciones_public_list');
    // GET lista publicaciones para home.
    Route::get('/publicaciones-home','PublicacionesController@listaHome')
        ->name('publicaciones_home_list');
    // GET vista publicación
    Route::get('/publicaciones/{categoria?}/{id?}','PublicacionesController@show')
        ->name('publicacion_show');
    ...
    Route::post('/solicitar-datos-egresado/{id}','MailsController@solicitarDatosEgresado')
        ->name('egresado_solicitar');
});
...
Route::group(['middleware' => ['web','auth','role:institucion'],'as' => 'institucion.'], function () {
    ...
    Route::get('/administrar-egresados','EgresadosController@showListadoInstitucion')
        ->name('admin_egresados');

    // GET AJAX lista egresados de la institucion (resp JSON)
    // Puede incluir filtros y ordenamiento como parametros get, y numero pagina
    Route::get('/egresados-institucion','EgresadosController@listaInstitucion')
        ->name('egresados_list');

    // GET AJAX: Busqueda nombre, apellido, especialidad (resp JSON)
    Route::get('/egresados-institucion/search','EgresadosController@searchInstitucion')
        ->name('egresados_search');

    /** Creacion, edicion, eliminación **/
    // GET pantalla formulario nuevo egresado
    Route::get('/administrar-egresados/nuevo','EgresadosController@nuevo')
        ->name('egresado_nuevo');
    // POST formulario nuevo egresado
    Route::post('/administrar-egresados/nuevo','EgresadosController@store')
        ->name('egresado_nuevo_post');
    // GET pantalla formulario editar egresado
    Route::get('/administrar-egresados/editar/{id?}','EgresadosController@edit')
        ->name('egresado_edit');
    // PUT formulario editar egresado
    Route::put('/administrar-egresados/edit/{id}','EgresadosController@update')
        ->name('egresado_edit_put');
    // GET AJAX para eliminar egresado
    Route::get('/administrar-egresados/delete/{id?}','EgresadosController@destroy')
...
Route::group(['middleware' => ['web','auth','role:admin'],'as' => 'publicaciones.'], function () {
    ...
    Route::get('/administrar-publicaciones','PublicacionesController@administrar')
        ->name('admin_publicaciones');

    /** Creacion, edicion, eliminación de publicaciones **/
    // GET pantalla formulario nueva publicación
    Route::get('/administrar-publicaciones/nueva','PublicacionesController@nueva')
        ->name('publicacion_nueva');
    // POST formulario nueva publicación
    Route::post('/administrar-publicaciones/nueva','PublicacionesController@store')
        ->name('publicacion_nueva_post');
    // GET pantalla formulario editar publicación
    Route::get('/administrar-publicaciones/editar/{id?}','PublicacionesController@edit')
        ->name('publicacion_edit');
    // PUT formulario editar publicación
    Route::put('/administrar-publicaciones/edit/{id}','PublicacionesController@update')
        ->name('publicacion_edit_put');
    // GET AJAX para eliminar publicación
    Route::get('/administrar-publicaciones/delete/{id?}','PublicacionesController@destroy')
